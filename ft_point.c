...
/*   Updated: 2023/03/13 10:20:00 by valarcon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_point(t_objet *obj, t_conf *conf)
{
	...
	if (obj->type == 1)
	{
		...
		if (intensity < 0)
			intensity = 0;
		rgb.red = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.red / 255) + (intensity/distance * (conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance)))  * aux->color.red;
        if (rgb.red > 255)
            rgb.red = 255;
        rgb.green = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.green / 255) + (intensity/distance * (conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance))) * aux->color.green;
        if (rgb.green > 255)
            rgb.green = 255;
        rgb.blue = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.blue / 255) + (intensity/distance * (conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance))) * aux->color.blue;
        if (rgb.blue > 255)
            rgb.blue = 255;
		tmp = rgb_to_int(rgb);
	...
	else if (obj->type == 2)
	{
		...
		if (aux3->base.plane_ecuation.x * aux3->intersection.x + aux3->base.plane_ecuation.y * aux3->intersection.y + aux3->base.plane_ecuation.z * aux3->intersection.z == aux3->base.plane_ecuation.x * aux3->base.center.x + aux3->base.plane_ecuation.y * aux3->base.center.y + aux3->base.plane_ecuation.z * aux3->base.center.z)
		{
			...
			if (intensity < 0)
				intensity = 0;
			      auxpoint = vec(aux3->intersection.x - conf->my_scene.light_lst.pos.x, aux3->intersection.y - conf->my_scene.light_lst.pos.y, aux3->intersection.z - conf->my_scene.light_lst.pos.z);
            distance = sqrt(auxpoint.x * auxpoint.x + auxpoint.y * auxpoint.y + auxpoint.z * auxpoint.z);
			 rgb.red = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.red / 255) + intensity/distance* conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->base.color.red;
            rgb.green = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.green / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->base.color.green;
            rgb.blue = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.blue / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->base.color.blue;
			tmp = rgb_to_int(rgb);
			conf->my_pixel.color = tmp;
		}
		else if (aux3->roof.plane_ecuation.x * aux3->intersection.x + aux3->roof.plane_ecuation.y * aux3->intersection.y + aux3->roof.plane_ecuation.z * aux3->intersection.z == aux3->roof.plane_ecuation.x * aux3->roof.center.x + aux3->roof.plane_ecuation.y * aux3->roof.center.y + aux3->roof.plane_ecuation.z * aux3->roof.center.z)
		{
			...
			if (intensity < 0)
			 ...
			 auxpoint = vec(aux3->intersection.x - conf->my_scene.light_lst.pos.x, aux3->intersection.y - conf->my_scene.light_lst.pos.y, aux3->intersection.z - conf->my_scene.light_lst.pos.z);
            distance = sqrt(auxpoint.x * auxpoint.x + auxpoint.y * auxpoint.y + auxpoint.z * auxpoint.z);
            rgb.red = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.red / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->roof.color.red;
            rgb.green = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.green / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->roof.color.green;
            rgb.blue = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.blue / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->roof.color.blue;
			tmp = rgb_to_int(rgb);
			conf->my_pixel.color = tmp;
		}
		else
		{
			...
			if (intensity < 0)
			 ...
			 auxpoint = vec(aux3->intersection.x - conf->my_scene.light_lst.pos.x, aux3->intersection.y - conf->my_scene.light_lst.pos.y, aux3->intersection.z - conf->my_scene.light_lst.pos.z);
            distance = sqrt(auxpoint.x * auxpoint.x + auxpoint.y * auxpoint.y + auxpoint.z * auxpoint.z);
             rgb.red = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.red / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->color.red;
            rgb.green = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.green / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->color.green;
            rgb.blue = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.blue / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux3->color.blue;
			tmp = rgb_to_int(rgb);
			conf->my_pixel.color = tmp;
		}
	}
	else if (obj->type == 3)
	{
		...
		if (intensity < 0)
			intensity = 0;
		  auxpoint = vec(aux2->intersection.x - conf->my_scene.light_lst.pos.x, aux2->intersection.y - conf->my_scene.light_lst.pos.y, aux2->intersection.z - conf->my_scene.light_lst.pos.z);
            distance = sqrt(auxpoint.x * auxpoint.x + auxpoint.y * auxpoint.y + auxpoint.z * auxpoint.z);
			  rgb.red = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.red / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux2->color.red;
            rgb.green = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.green / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux2->color.green;
            rgb.blue = ((conf->my_scene.ambient.radius * conf->my_scene.ambient.color.blue / 255) + intensity/distance * conf->my_scene.light_lst.radius * conf->my_scene.light_lst.radius/distance) * aux2->color.blue;
		tmp = rgb_to_int(rgb);
		conf->my_pixel.color = tmp;
	}
}
